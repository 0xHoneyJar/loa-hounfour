...
**Source**: Review — PR #37, Strategic Question Q1

## Context

The Commons Protocol v8.0.0 established `GovernedResource<T>` as a governance substrate with schemas for conservation laws, audit trails, state machines, and dynamic contracts. The review identified a key strategic question:
...
1. **consumer-c** enforces `lot_invariant` with its own `validateLotConservation()` — duplicating what the protocol could provide canonically.
2. **consumer-b** built a full `ResourceGovernor<T>` abstraction — the most advanced local implementation.
3. **consumer-a** uses `evaluateAccessPolicy()` (v7.2.0) — an existing enforcement utility that proves the pattern.
...
| Utility | Schema it enforces | Finding |
|---------|-------------------|----------------------|
...
- Review, PR #37 — "Schema library vs governance framework (Path A vs B)"
...
- consumer-b `ResourceGovernor<T>` — most advanced local enforcement pattern
