...
| `signer_key_id` | Lowercase 64-hex SHA-256 of `signer_cluster_id || ':' || signer_key_version` (UTF-8 bytes, no NFC at the derivation layer). | The `signer_key_id_matches_derivation` LOCAL builtin enforces this invariant; case-insensitive comparison admits uppercase hex on the wire. |
