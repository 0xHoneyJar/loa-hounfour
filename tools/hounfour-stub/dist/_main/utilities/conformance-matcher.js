 ...
 */
export function matchConformanceOutput(expected, actual, rules) {
    try {
        const mismatches = [];
        ...
        if (rules?.volatile_fields?.length) {
            fieldsToCompare = fieldsToCompare.filter((f) => !rules.volatile_fields.includes(f));
        }
    ...
    if (typeof a === 'object' && typeof b === 'object') {
        ...
        for (const key of allKeys) {
            ...
            if (!(key in aObj) && key in bObj) {
                if (bVal === null && rules?.null_handling !== 'strict')
                    continue;
                return false;
            }
            if (key in aObj && !(key in bObj)) {
                if (aVal === null && rules?.null_handling !== 'strict')
                    continue;
                return false;
            }
            if (!deepEqual(aVal, bVal, rules))
                return false;
        }
        return true;
    }
...
//# sourceMappingURL=conformance-matcher.js.map