 ...
 */
import { isValidTransition } from '../schemas/agent-lifecycle.js';
...
export class ProtocolStateTracker {
     ...
     */
    applyStateMachineEvent(machineId, stateMap, entityId, targetState, notFoundReason, invalidTransitionReason) {
    ...
    applyLifecycleTransition(event) {
    ...
    applyEconomyEvent(event) {
        const payload = event.payload;
        // --- Escrow events ---
        if (event.type === 'economy.escrow.created') {
            const escrowId = payload.escrow_id || event.aggregate_id;
        ...
        if (event.type.startsWith('economy.escrow.')) {
            ...
            return this.applyStateMachineEvent('escrow', this.escrowStates, escrowId, targetState, 'escrow_not_found', 'invalid_escrow_transition');
        }
        // --- Stake events ---
        if (event.type === 'economy.stake.created') {
            const stakeId = payload.stake_id || event.aggregate_id;
        ...
        if (event.type.startsWith('economy.stake.')) {
            const stakeId = payload.stake_id || event.aggregate_id;
            const targetState = event.type.split('.')[2];
            return this.applyStateMachineEvent('stake', this.stakeStates, stakeId, targetState, 'stake_not_found', 'invalid_stake_transition');
        }
        // --- Credit events ---
        if (event.type === 'economy.credit.extended') {
            const creditId = payload.credit_id || event.aggregate_id;
            this.creditStates.set(creditId, STATE_MACHINES.credit.initial);
            return { applied: true };
        }
        if (event.type === 'economy.credit.settled') {
            const creditId = payload.credit_id || event.aggregate_id;
            return this.applyStateMachineEvent('credit', this.creditStates, creditId, 'settled', 'credit_not_found', 'invalid_credit_transition');
        }
        // Unknown economy events are accepted without state changes
        return { applied: true };
    }
}
//# sourceMappingURL=protocol-state-tracker.js.map