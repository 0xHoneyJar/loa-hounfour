...
export const MerkleProofStepSchema = Type.Object({
    sibling_hash: Type.String({
        ...
        description: 'SHA-256 digest of the sibling node, in canonical ' +
    ...
    position: Type.Union([Type.Literal('left'), Type.Literal('right')], {
        description: 'Position discriminator pinning which side of the ' +
    ...
    description: 'One per-step entry within a Merkle proof path. Composition ' +
...
export const ProposedSeriesGoalSchema = Type.Object({
    id: Type.String({
        minLength: 1,
        description: 'Stable opaque goal identifier within the artifact. Per-' +
    ...
    description: Type.String({
        minLength: 1,
        maxLength: 4096,
        description: 'Free-form goal description. 4 KiB upper bound matches the ' +
    ...
    conformance_impact_pct: Type.Number({
        minimum: -100,
        maximum: 100,
        description: 'Estimated conformance percentage delta if this goal lands. ' +
    ...
    additionalProperties: false,
    description: 'One per-goal entry within an InterSeriesScopingArtifact. ' +
...
export const InterSeriesScopingArtifactSchema = Type.Object({
    ...
    ts: Type.String({
        ...
        description: 'ISO 8601 UTC timestamp at which this scoping artifact was ' +
    ...
    parent_series_id: Type.String({
        minLength: 1,
        description: 'Lazy-link to ClusterRunSeries.run_id (FR-G1) of the parent ' +
            'series this scoping artifact applies to. Resolution is ' +
            'consumer-state per ADR-010 — hounfour does not bundle a ' +
            'registry implementation.',
    }),
    signoff_envelope_id: Type.Union([Type.String({ minLength: 1 }), Type.Null()], {
        description: 'Optional lazy-link to a PlanSignoffEnvelopeSchema record ' +
    ...
    proposed_series_goals: Type.Array(ProposedSeriesGoalSchema, {
        minItems: 1,
        maxItems: 64,
        description: 'Non-empty array of proposed goals for the scoped series. ' +
    ...
    constitutional_hash_proof: Type.Object({
        soul_hash: Type.String({
            pattern: SHA256_HEX_PATTERN,
            description: 'Leaf hash — typically the canonical-form hash of the ' +
        ...
        proof_path: Type.Array(MerkleProofStepSchema, {
            minItems: 0,
            maxItems: 64,
            description: 'Ordered sequence of Merkle proof steps from leaf to ' +
        ...
        additionalProperties: false,
        description: 'Merkle proof anchoring the parent series to a consumer-' +
            'published constitutional-root. Shape-level only; semantic ' +
            'root verification is consumer-state per ADR-010.',
    }),
    conformance_chain: Type.Array(Type.String({ pattern: SHA256_HEX_PATTERN }), {
        minItems: 0,
        maxItems: 1024,
        description: 'Ordered sequence of conformance-evaluation hashes; ' +
    ...
    additionalProperties: false,
    description: 'Cross-EPIC scoping artifact. Merkle proof composition is ' +
 ...
 * @since v8.7.0 — FR-G2 (PR-A4.2).
 */
export function validateInterSeriesScopingArtifact(data) {
    const errors = [];
    ...
    const goals = data.proposed_series_goals;
    if (!Array.isArray(goals)) {
        errors.push('ISSA: structural shape precondition failed — proposed_series_goals must be a non-null array; the cross-field validator requires the structural tier to have passed first.');
    }
    else {
        const distinct = arrayFieldDistinct(goals, 'id');
        if (!distinct.valid) {
            for (const dup of distinct.duplicates) {
                errors.push(`ISSA-2: proposed_series_goals[*].id "${dup.value}" appears at indices ${dup.indices.join(', ')} — id MUST be distinct within an artifact.`);
            }
        }
    }
    ...
    const proof = data.constitutional_hash_proof;
    if (proof === null || typeof proof !== 'object' || Array.isArray(proof)) {
        errors.push('ISSA: structural shape precondition failed — constitutional_hash_proof must be a non-null object; the cross-field validator requires the structural tier to have rejected this envelope first.');
    }
    else {
        ...
        if (!Array.isArray(proofPath)) {
            ...
            errors.push(`ISSA: structural shape precondition failed — constitutional_hash_proof.proof_path must be a non-null array; got ${proofPath === null ? 'null' : proofPath === undefined ? 'undefined' : typeof proofPath}. The cross-field validator requires the structural tier (Value.Check) to have rejected this envelope first.`);
        }
        else {
...
//# sourceMappingURL=inter-series-scoping-artifact.js.map