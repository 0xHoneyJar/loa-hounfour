 ...
 */
export function evaluatePlanContentHashUnchangedSinceSignoff(planHash, ledgerSnapshot) {
    if (typeof planHash !== 'string') {
        return {
            result: 'fail',
            manifestEntry: {
                rule_id: PLAN_CONTENT_HASH_RULE_ID,
                rule: `plan_content_hash_unchanged_since_signoff: plan_hash argument ` +
                    `must be a string; received ${typeof planHash}.`,
                evaluator: 'library',
                evaluation_note: 'plan_content_hash_unchanged_since_signoff received a non-string ' +
    ...
    if (ledgerSnapshot === null || ledgerSnapshot === undefined) {
        return {
            ...
            manifestEntry: {
                rule_id: PLAN_CONTENT_HASH_RULE_ID,
                rule: `plan_content_hash_unchanged_since_signoff(plan_content_hash) ` +
                    `with ledger snapshot absent.`,
                evaluator: 'consumer',
                reason: 'ledger_context_deferred',
                evaluation_note: 'plan_content_hash_unchanged_since_signoff: signoff ledger ' +
    ...
    if (!Array.isArray(ledgerSnapshot.signoffs)) {
        return {
            ...
            manifestEntry: {
                rule_id: PLAN_CONTENT_HASH_RULE_ID,
                rule: `plan_content_hash_unchanged_since_signoff: ` +
                    `ledgerSnapshot.signoffs must be an array; received ` +
                    `${typeof ledgerSnapshot.signoffs}.`,
                evaluator: 'library',
                evaluation_note: 'plan_content_hash_unchanged_since_signoff received a ' +
    ...
    const matching = ledgerSnapshot.signoffs.find((s) => s.plan_content_hash.toLowerCase() === planHashLc);
    if (matching === undefined) {
        return {
            ...
            manifestEntry: {
                ...
                evaluation_note: `plan_content_hash_unchanged_since_signoff: plan_hash ` +
    ...
    if (!Number.isFinite(ts_emit_ms)) {
        return {
            ...
            manifestEntry: {
                ...
                evaluator: 'library',
                evaluation_note: 'plan_content_hash_unchanged_since_signoff: the matching ' +
    ...
    return {
        ...
        manifestEntry: {
            ...
            evaluation_note: `plan_content_hash_unchanged_since_signoff: signoff matches ` +
...
//# sourceMappingURL=plan-content-hash-unchanged-since-signoff.js.map