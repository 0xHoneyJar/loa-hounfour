 ...
 * @since v8.7.0 — FR-G2 (PR-A4.2).
 */
export function merkleProofCompositionWellFormed(proofPath) {
    ...
    for (let i = 0; i < proofPath.length; i += 1) {
        ...
        if (step === null || typeof step !== 'object' || Array.isArray(step)) {
            issues.push(`proof_path[${i}] is ${step === null ? 'null' : Array.isArray(step) ? 'array' : typeof step}, not an object — Merkle proof step well-formedness requires a non-null object with position and sibling_hash fields.`);
            continue;
        }
        const stepObj = step;
        const position = stepObj.position;
        if (position !== 'left' && position !== 'right') {
            issues.push(`proof_path[${i}].position is ${typeof position === 'string' ? `"${position}"` : String(position)} — position MUST be exactly "left" or "right" to disambiguate the matching-twins composition trace.`);
        }
        const siblingHash = stepObj.sibling_hash;
        if (typeof siblingHash !== 'string') {
            issues.push(`proof_path[${i}].sibling_hash is ${siblingHash === null ? 'null' : typeof siblingHash}, not a string — sibling_hash MUST match the canonical sha256:<64-hex> form.`);
        }
        else if (!SHA256_HEX_REGEX.test(siblingHash)) {
            issues.push(`proof_path[${i}].sibling_hash "${siblingHash}" does not match the canonical sha256:<64-hex> form — Merkle composition operates on the raw 32-byte digest decoded from the hex form.`);
        }
    }
    return { valid: issues.length === 0, issues };
}
//# sourceMappingURL=inter-series-scoping-artifact-local.js.map