...
export const RecallReceiptSchema = Type.Object({
    ...
    pack_hash: Type.String({
        ...
        description: 'Reference-by-value to RecallPack.pack_hash. Recursive content NOT included in receipt_hash — the receipt commits to the pack via this hash literal.',
    }),
    detail_level: ReceiptDetailLevelSchema,
    signature_envelope: SignatureEnvelopeSchema,
    receipt_hash: Type.String({
        pattern: '^sha256:[0-9a-f]{64}$',
        description: 'SHA-256 hex digest (lowercase) of safeCanonicalize(receipt-minus-receipt_hash). NFC + RFC 8785 + 100KB cap per the hashing-spec freeze.',
    }),
    ...
    description: 'Crypto-bearing signed acknowledgment for a RecallPack. validate() defaults to { valid: false, errors: [CRYPTO_DEFERRED] } unless { acceptDeferred: true } is passed; consumer must verify the wrapped signature_envelope and reconcile pack_hash + receipt_hash before treating the receipt as authoritative.',
});
//# sourceMappingURL=recall-receipt.js.map