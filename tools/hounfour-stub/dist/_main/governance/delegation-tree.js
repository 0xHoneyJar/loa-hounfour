...
export function chainToTree(chain) {
    ...
    for (let i = links.length - 1; i >= 0; i--) {
        const link = links[i];
        const child = nodes.length > 0 ? nodes[nodes.length - 1] : undefined;
        const node = {
            node_id: uuid(),
 ...
 */
export function treeToChain(tree) {
    const links = [];
    let current = tree.root;
    while (current.children.length > 0) {
        if (current.children.length > 1)
            return null; // branching → not a chain
...
//# sourceMappingURL=delegation-tree.js.map