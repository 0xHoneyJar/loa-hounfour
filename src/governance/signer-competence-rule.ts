...
export const SignerCompetenceRuleSchema = Type.Object(
  {
    ...
    action_pattern: Type.String({
      ...
      maxLength: 256,
      description:
        'Action descriptor matched against incoming action references. Consumer-defined matching strategy (glob, regex, prefix); hounfour does not interpret.',
    }),
    required_capability_scopes: Type.Array(CapabilityScopeSchema, {
      minItems: 1,
      description:
        'Capability scopes a signer\'s trust profile MUST satisfy for this rule to authorize. Bind to the canonical CapabilityScope vocabulary; consumer-side extensions trigger an ORD-5 vocabulary_drift manifest entry (warn mode).',
    }),
    threshold: Type.Optional(
      Type.Integer({
        ...
        maximum: 1000,
        description:
          'Optional minimum precedence_score for matching SignerEntry profiles. Used when multiple signers may satisfy a rule; consumers without precedence semantics can ignore.',
      }),
    ),
    revoked: Type.Optional(
      Type.Boolean({
        description:
          'When true, the rule no longer applies regardless of pattern match. Consumers MUST treat revoked rules as if absent.',
      }),
    ),
  ...
  },
  {
    ...
    description:
      'Layer-3 rule pairing an action pattern with the capability-scope set required to authorize matching actions. Composes with Layer-2 SignerEntry.scoped_trust.',
  },
);
