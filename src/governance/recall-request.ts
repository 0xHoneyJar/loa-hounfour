...
export const RecallRequestSchema = Type.Object(
  {
    ...
    requestor_signer_id: Type.Optional(
      Type.String({
        pattern: '^[a-z][a-z0-9_-]{2,63}$',
        description:
          'Optional consumer-side signer-id reference. Hounfour ships shape only; consumer dereferences against its Keyring + verifies any wrapping signature envelope.',
      }),
    ),
  ...
  },
  {
    ...
    description:
      'Input record for recall machinery. Carries subject_agent_id, surface_context (env scope), detail_level (verbosity selector for the matching RecallReceipt), and an Optional requestor_signer_id for consumer-side signing.',
  },
);
