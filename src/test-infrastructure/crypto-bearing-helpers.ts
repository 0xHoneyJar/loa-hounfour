...
export function assertCryptoBearingFailsByDefault<T extends TSchema>(
  ...
  const hasVariantFlag =
    ...
    (schemaRecord.anyOf as Array<Record<string, unknown>>).some(
      (variant) => variant['x-crypto-bearing'] === true,
    );
  if (!topLevelFlag && !hasVariantFlag) {
    const id = schemaRecord.$id ?? '<anonymous>';
    throw new Error(
      `assertCryptoBearingFailsByDefault: ${String(id)} is NOT marked x-crypto-bearing (top-level or variant-aware) — call assertStructurallyValid for non-crypto schemas.`,
    );
  }
