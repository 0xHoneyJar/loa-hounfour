...
export const PhaseCompletionEnvelopeSchema = Type.Object(
  {
    envelope_kind: Type.Literal('phase_completion', {
      description:
        'Discriminator literal pinning the Tier-2 cluster-wrapped shape. ' +
    ...
    signer_cluster_id: Type.String({
      minLength: 1,
      description:
        'Stable cluster identifier (consumer-shaped — hounfour does not ' +
    ...
    signer_key_version: Type.String({
      pattern: '^[1-9][0-9]*$',
      description:
        'String-encoded ≥1 integer (CT-03; consumer parses to BigInt ' +
    ...
    signer_key_id: Type.String({
      ...
      description:
        'Derived sha256 hex (bare 64-char form — no `sha256:` prefix; ' +
    ...
    prev_envelope_hash: Type.String({
      ...
      description:
        'sha256:<64-hex> hash of the previous envelope\'s canonical-JSON ' +
    ...
    ingest_timestamp: Type.String({
      ...
      description:
        'ISO 8601 UTC timestamp at cluster ingestion (Z suffix). ' +
  ...
  },
  {
    ...
    description:
      'Cluster-wrapped Tier-2 phase-completion envelope. Wraps Tier-1 ' +
