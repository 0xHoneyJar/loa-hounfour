 ...
 * @since v8.7.0 — FR-G1 (PR-A4.1).
 */
export function arrayFieldDistinct(
  array: unknown,
  fieldName: string,
): {
  valid: boolean;
  duplicates: Array<{ value: string; indices: number[] }>;
} {
  ...
  const duplicates: Array<{ value: string; indices: number[] }> = [];
 ...
 * @since v8.7.0 — FR-G1 (PR-A4.1).
 */
export function failureModeIffFailedStatus(
  epicStatus: unknown,
  failureMode: unknown,
  ...
  if (isFailed && !hasFailureMode) {
    return {
      valid: false,
      reason:
        'epic_status is "failed" but failure_mode is null — failed entries MUST carry a failure_mode classifier (CRS-2).',
    };
  }
  if (!isFailed && hasFailureMode) {
    return {
