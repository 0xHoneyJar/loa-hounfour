...
import {
  canonicalByteLength,
  CANONICAL_SIZE_CAP_BYTES,
} from '../../utilities/canonical-size-cap.js';

export type CanonicalSizeCapErrorCode =
  | 'CANONICAL_SIZE_CAP_EXCEEDED'
  | 'CANONICAL_SIZE_CAP_INVALID_INPUT';
 ...
 */
export function evaluateCanonicalSizeCap(
  value: unknown,
  byteCap: number = CANONICAL_SIZE_CAP_BYTES,
): EvaluateCanonicalSizeCapResult {
  if (
  ...
  try {
    ...
    return {
      ...
      diagnostic: {
        code: 'CANONICAL_SIZE_CAP_INVALID_INPUT',
        message:
          `canonical_size_cap: safeCanonicalize rejected the input — ` +
  ...
  if (actualBytes > byteCap) {
    return {
      ...
      diagnostic: {
        ...
        message:
          `canonical_size_cap: canonical-JSON byte length ${actualBytes} ` +
