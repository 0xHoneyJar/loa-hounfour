...
const AgentStatsSchema = Type.Object(
  {
    ...
    last_active: Type.Optional(
      Type.String({ format: 'date-time', description: 'Last activity timestamp' }),
    ),
  },
...
export const AgentDescriptorSchema = Type.Object(
  {
    ...
    // On-chain
    tba: Type.Optional(
      Type.String({ pattern: '^0x[a-fA-F0-9]{40}$', description: 'ERC-6551 TBA address' }),
    ),
    owner: Type.Optional(
      Type.String({ pattern: '^0x[a-fA-F0-9]{40}$', description: 'Current NFT owner address' }),
    ),
    ...
    llms_txt: Type.Optional(
      Type.String({ format: 'uri', description: 'Token-efficient markdown endpoint' }),
    ),
