...
export const CapabilityScopedTrustSchema = Type.Object(
  {
    ...
    match_strategy: Type.Optional(
      Type.Union(
        [Type.Literal('exact'), Type.Literal('subset'), Type.Literal('superset')],
        {
          description:
            'How a SignerCompetenceRule.required_capability_scopes set is compared against this profile\'s scopes. Default behavior on absence is "subset" (the trust profile must include every required scope) — preserves v8.4.0 implicit semantics.',
        },
      ),
    ),
    precedence_score: Type.Optional(
      Type.Integer({
        minimum: 0,
        maximum: 1000,
        description:
          'Tiebreaker when multiple SignerCompetenceRules match the same action. Higher score wins. Absence means "no preference" — consumers fall back to rule-list order.',
      }),
    ),
  },
  {
    ...
    additionalProperties: false,
    description:
      'Per-capability-scope trust levels with default fallback (v6.0.0; v8.5.0 EXTEND adds optional match_strategy + precedence_score per D-007 / OQ-S6).',
  },
);
