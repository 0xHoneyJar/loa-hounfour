...
export const SANCTION_SEVERITY_LADDER: readonly SeverityLadderEntry[] = [
  {
    severity: 'warning',
    level: 1,
 ...
 */
export function getSeverityEntry(severity: SanctionSeverityLevel): SeverityLadderEntry {
  const entry = SANCTION_SEVERITY_LADDER.find(e => e.severity === severity);
  if (!entry) throw new Error(`Unknown severity level: ${severity}`);
  return entry;
}
 ...
 */
export function compareSeverity(a: SanctionSeverityLevel, b: SanctionSeverityLevel): number {
