...
export class CanonicalizeSizeError extends Error {
  ...
  readonly code = 'CANONICALIZE_SIZE_EXCEEDED' as const;
  ...
  constructor(serializedBytes: number, maxBytes: number) {
    super(
      `Canonicalized payload is ${serializedBytes} bytes; exceeds ${maxBytes}-byte cap. ` +
...
export class CanonicalizeNFCError extends Error {
  readonly code = 'CANONICALIZE_NFC_FAILED' as const;
 ...
 */
function nfcNormalizeTree(value: unknown): unknown {
  if (typeof value === 'string') {
    try {
      return value.normalize('NFC');
    } catch (e) {
      throw new CanonicalizeNFCError(
        'NFC normalization failed; payload contains malformed Unicode.',
        e,
      );
    }
  }
 ...
 */
export function safeCanonicalize(
  payload: unknown,
  options: SafeCanonicalizeOptions = {},
): string {
  ...
  if (typeof canonical !== 'string') {
    ...
    throw new TypeError(
      `safeCanonicalize: payload is not representable as canonical JSON ` +
