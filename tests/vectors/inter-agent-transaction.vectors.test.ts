/**
 * Tests for InterAgentTransactionAudit conformance vectors (S1-T7).
 *
...
const vectorDir = join(rootDir, 'vectors', 'conformance', 'inter-agent-transaction');
...
describe('InterAgentTransactionAudit conformance vectors', () => {
  ...
  for (const file of vectorFiles) {
    ...
    describe(vector.vector_id, () => {
      it(`schema validation matches expected_valid (${vector.expected_valid})`, () => {
        const schemaValid = Value.Check(InterAgentTransactionAuditSchema, vector.input);
