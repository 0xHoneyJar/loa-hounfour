...
describe('Version Compatibility (v5.0.0)', () => {
  ...
  it('patch difference is fully compatible', () => {
    const result = validateCompatibility('5.0.1');
    expect(result.compatible).toBe(true);
  });
  ...
  it('below MIN_SUPPORTED_VERSION is incompatible', () => {
    const result = validateCompatibility('3.9.0');
  ...
  it('version constants match expected values', () => {
    ...
    expect(MIN_SUPPORTED_VERSION).toBe('4.0.0');
  });
});
