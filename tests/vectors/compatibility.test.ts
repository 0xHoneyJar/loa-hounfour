...
describe('Version Compatibility (v4.0.0)', () => {
  ...
  it('patch difference is fully compatible', () => {
    const result = validateCompatibility('4.0.1');
    expect(result.compatible).toBe(true);
  });
  ...
  it('below MIN_SUPPORTED_VERSION is incompatible', () => {
    const result = validateCompatibility('2.9.0');
  ...
  it('same major, different minor is compatible with warning', () => {
    const result = validateCompatibility('4.1.0');
    expect(result.compatible).toBe(true);
  ...
  it('version constants match expected values', () => {
    expect(CONTRACT_VERSION).toBe('4.0.0');
