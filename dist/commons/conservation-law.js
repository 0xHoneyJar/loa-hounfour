...
export const ConservationLawSchema = Type.Object({
    invariants: Type.Array(InvariantSchema, {
        description: 'Named expressions that must hold before/after state transitions. '
            + 'When enforcement is strict, at least one invariant is required.',
    }),
    enforcement: Type.Union([Type.Literal('strict'), Type.Literal('advisory')], {
        description: 'strict: violation halts and records audit event. '
            + 'advisory: violation warns and records audit event.',
    }),
    scope: Type.Union([Type.Literal('per-entry'), Type.Literal('aggregate')], {
        description: 'per-entry: evaluated per state transition. '
...
//# sourceMappingURL=conservation-law.js.map