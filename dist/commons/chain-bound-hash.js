 ...
 */
export function validateDomainTag(domainTag) {
 ...
 */
export function computeChainBoundHash(entry, domainTag, previousHash) {
    // Validate domain tag
    ...
    if (!SHA256_HASH_PATTERN.test(previousHash)) {
        throw new ChainBoundHashError('INVALID_PREVIOUS_HASH', `Previous hash must match sha256:<64 hex> format, got "${previousHash}"`);
    }
    ...
    try {
        ...
        throw new ChainBoundHashError('CANONICALIZATION_FAILED', 'Failed to canonicalize audit entry for content hash');
    }
...
//# sourceMappingURL=chain-bound-hash.js.map