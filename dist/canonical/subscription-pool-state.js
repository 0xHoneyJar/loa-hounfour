...
import { stringMicroUsdLe, iso8601GeField, } from '../constraints/builtins/subscription-pool-state-local.js';
...
export const SubscriptionAccountStateSchema = Type.Union([
    ...
    description: 'Per-account health state across a subscription pool. Locked ' +
...
export const RateEnvelopeSchema = Type.Object({
    req_per_min: Type.Integer({
        minimum: 0,
        maximum: 1_000_000,
        description: 'Per-minute request allowance for the account. Integer with ' +
    ...
    req_per_hour: Type.Integer({
        minimum: 0,
        maximum: 60_000_000,
        description: 'Per-hour request allowance for the account. Integer with ' +
    ...
    description: 'Per-account rate-limit envelope. Library-clamped integer ' +
...
export const SubscriptionAccountEntrySchema = Type.Object({
    account_id: Type.String({
        minLength: 1,
        description: 'Stable opaque account identifier within the pool. Per-array ' +
    ...
    provider: Type.String({
        minLength: 1,
        maxLength: 64,
        description: 'Free-form provider classifier (e.g. "anthropic", "openai"). ' +
    ...
    stable_until: Type.String({
        ...
        description: 'ISO 8601 UTC timestamp after which the account state may ' +
    ...
    thrash_count_24h: Type.Integer({
        minimum: 0,
        maximum: 100_000,
        description: '24-hour rolling count of state-thrash events. Closed upper ' +
    ...
    allocated_units: Type.String({
        ...
        description: 'String-encoded micro-USD per CLAUDE.md financial-value ' +
    ...
    consumed_units: Type.String({
        pattern: MICRO_USD_PATTERN,
        description: 'String-encoded micro-USD. MUST be at-or-below allocated_units ' +
    ...
    additionalProperties: false,
    description: 'One per-account entry within a SubscriptionPoolState. Cross-' +
...
export const SubscriptionPoolStateSchema = Type.Object({
    ...
    ts: Type.String({
        pattern: ISO8601_UTC_PATTERN,
        description: 'ISO 8601 UTC timestamp at which this pool-state snapshot was ' +
    ...
    pool_health_id: Type.Union([Type.String({ minLength: 1 }), Type.Null()], {
        description: 'Optional lazy-link to an OracleHealthEnvelopeSchema record ' +
    ...
    accounts: Type.Array(SubscriptionAccountEntrySchema, {
        minItems: 1,
        maxItems: 256,
        description: 'Non-empty array of per-account state entries. Per-array ' +
    ...
    pause_resume_pair_id: Type.String({
        minLength: 1,
        description: 'Idempotency marker pairing each pause with its corresponding ' +
    ...
    signer_cluster_id: Type.String({
        minLength: 1,
        description: 'Consumer-shaped identifier for the signing cluster. SPS-3 ' +
    ...
    signature: Type.String({
        ...
        description: 'Ed25519 signature over the canonicalized payload (consumer-' +
    ...
    description: 'Pool-state snapshot for shared-capacity audits. Crypto-bearing ' +
 ...
 * @since v8.7.0 — FR-G3 (PR-A4.3).
 */
export function validateSubscriptionPoolState(data) {
    const errors = [];
    ...
    for (let i = 0; i < accounts.length; i += 1) {
        ...
        if (entry === null || typeof entry !== 'object' || Array.isArray(entry)) {
            errors.push(`SPS: accounts[${i}] is ${entry === null ? 'null' : Array.isArray(entry) ? 'array' : typeof entry}, not an object — cross-field validator requires the structural tier to have rejected this element first.`);
            continue;
        }
        const consumed = entry.consumed_units;
        const allocated = entry.allocated_units;
        const sps1 = stringMicroUsdLe(consumed, allocated);
        if (!sps1.valid) {
            errors.push(`SPS-1: accounts[${i}] ${sps1.reason}`);
        }
        const stableUntil = entry.stable_until;
    ...
    if (!distinct.valid) {
        for (const dup of distinct.duplicates) {
            errors.push(`SPS-2: accounts[*].account_id "${dup.value}" appears at indices ${dup.indices.join(', ')} — account_id MUST be distinct within a pool.`);
        }
    }
    return { valid: errors.length === 0, errors, warnings: [] };
}
//# sourceMappingURL=subscription-pool-state.js.map