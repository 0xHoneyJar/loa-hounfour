...
export const RevocationReasonSchema = Type.Union([
    ...
    $id: 'RevocationReason',
    description: 'Locked 5-member revocation classifier. Promotion to a ' +
...
export const QuorumSignatureEntrySchema = Type.Object({
    ...
    signature: Type.String({
        ...
        description: 'ed25519 signature over the JCS-canonical-form payload with ' +
    ...
    description: 'One per-signer entry in the optional quorum-of-signatures ' +
...
export const RevocationListEntrySchema = Type.Object({
    key_id: Type.String({
        pattern: ED25519_PUBKEY_PATTERN,
        description: 'ed25519 public-key identifier being revoked. 32-byte ' +
    ...
    revoked_at: Type.String({
        ...
        description: 'ISO 8601 UTC timestamp at which the key was revoked. RL-7 ' +
    ...
    revocation_evidence_hash: Type.Union([Type.String({ pattern: SHA256_HEX_PATTERN }), Type.Null()], {
        description: 'Optional SHA-256 of consumer-policy evidence document — ' +
    ...
    additionalProperties: false,
    description: 'One per-key entry within a RevocationList. Cross-element ' +
...
export const RevocationListSchema = Type.Object({
    ...
    list_id: Type.String({
        minLength: 1,
        description: 'Stable opaque list identifier. Consumer-shaped namespace; ' +
            'hounfour does not freeze the registry.',
    }),
    issued_at: Type.String({
        pattern: ISO8601_UTC_PATTERN,
        description: 'ISO 8601 UTC timestamp at which this list was issued. ' +
            'RL-7 and RL-10 use this as the upper bound for per-element ' +
            'revoked_at and the envelope valid_from respectively.',
    }),
    valid_from: Type.String({
        pattern: ISO8601_UTC_PATTERN,
        description: 'Effective-from timestamp for this list. RL-10 enforces ' +
    ...
    valid_until: Type.Union([Type.String({ pattern: ISO8601_UTC_PATTERN }), Type.Null()], {
        description: 'Optional expiry. Null means "until superseded by next ' +
            'nonce" — admissible per the anti-finding-rotation ' +
            'lock. RL-9 enforces valid_from at-or-before valid_until ' +
            'when both non-null.',
    }),
    max_staleness_seconds: Type.Union([Type.Integer({ minimum: 1, maximum: 31_536_000 }), Type.Null()], {
        description: 'Verifier freshness policy hint, in seconds (1 second to ' +
    ...
    nonce: Type.Integer({
        ...
        description: 'Monotonic per cluster_id per the v8.6.0 ' +
    ...
    prev_envelope_hash: Type.Union([Type.String({ pattern: SHA256_HEX_PATTERN }), Type.Null()], {
        description: 'Chain-prev hash; null only for the genesis envelope (the ' +
            'first list in a chain). RL-3 (consumer-state) verifies ' +
            'the chain via the v8.6.0 chain_validator_prev_hash builtin.',
    }),
    next_page_hash: Type.Union([Type.String({ pattern: SHA256_HEX_PATTERN }), Type.Null()], {
        description: 'Pagination forward-link. Null means "this is the last ' +
    ...
    revoked_keys: Type.Array(RevocationListEntrySchema, {
        minItems: 0,
        maxItems: 4096,
        description: 'Per-key revocation entries. minItems 0 is admissible ' +
    ...
    signature: Type.String({
        pattern: ED25519_SIGNATURE_PATTERN,
        description: 'ed25519 signature over the JCS-canonical-form bytes of the ' +
    ...
    description: 'Cluster verifiers MUST consult before accepting envelopes ' +
 ...
 * @since v8.7.0 — FR-G4 (PR-A4.4).
 */
export function validateRevocationList(data) {
    const errors = [];
    ...
    const signerKeyId = data.signer_key_id;
    const revokedKeys = data.revoked_keys;
    const quorumSignatures = data
        .quorum_signatures;
    if (!Array.isArray(revokedKeys)) {
        errors.push('RL: structural shape precondition failed — revoked_keys must be an array; the cross-field validator requires the structural tier (Value.Check) to have passed first.');
    ...
    if (!distinct.valid) {
        for (const dup of distinct.duplicates) {
            errors.push(`RL-1: revoked_keys[*].key_id "${dup.value}" appears at indices ${dup.indices.join(', ')} — key_id MUST be distinct within a single revocation list.`);
        }
    }
    ...
    const selfCheck = fieldNotInArrayField(signerKeyId, revokedKeys, 'key_id');
    if (!selfCheck.valid) {
        errors.push(`RL-5: signer_key_id "${signerKeyId}" appears in revoked_keys[${selfCheck.matchedIndex}].key_id — a key cannot revoke itself within the envelope it signs. Use a successor key to revoke the predecessor.`);
    }
    ...
    for (let i = 0; i < revokedKeys.length; i += 1) {
        ...
        if (entry === null || typeof entry !== 'object' || Array.isArray(entry)) {
            errors.push(`RL: revoked_keys[${i}] is ${entry === null ? 'null' : Array.isArray(entry) ? 'array' : typeof entry}, not an object — cross-field validator requires the structural tier to have rejected this element first.`);
            continue;
        }
    ...
    if (quorumSignatures !== null && quorumSignatures !== undefined) {
        if (!Array.isArray(quorumSignatures)) {
            errors.push(`RL: structural shape precondition failed — quorum_signatures must be null OR an array; got ${typeof quorumSignatures}.`);
        }
        else {
            const quorumDistinct = arrayFieldDistinct(quorumSignatures, 'signer_key_id');
            if (!quorumDistinct.valid) {
                for (const dup of quorumDistinct.duplicates) {
                    errors.push(`RL-12: quorum_signatures[*].signer_key_id "${dup.value}" appears at indices ${dup.indices.join(', ')} — quorum signers MUST be distinct.`);
                }
            }
            const memberCheck = fieldInArrayField(signerKeyId, quorumSignatures, 'signer_key_id');
            if (!memberCheck.valid) {
                errors.push(`RL-12: primary signer_key_id "${signerKeyId}" is not present in quorum_signatures[*].signer_key_id — when quorum_signatures is non-null, the primary signer MUST also appear in the quorum.`);
            }
        }
    }
