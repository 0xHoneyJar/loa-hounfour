 ...
 */
export function evaluateNonceUniquePerSignerWindow(record, signerIdField, nonceField, state) {
    ...
    if (typeof signerId !== 'string' || typeof nonce !== 'string') {
        return {
            ...
            diagnostic: {
                code: 'NONCE_INVALID_INPUT',
                message: `nonce_unique_per_signer_window: ${signerIdField} and ${nonceField} ` +
    ...
    if (state !== undefined && !(state.per_signer instanceof Map)) {
        return {
            ...
            diagnostic: {
                code: 'NONCE_INVALID_INPUT',
                message: 'nonce_unique_per_signer_window: state.per_signer must be a Map ' +
    ...
    if (state !== undefined &&
        ...
        return {
            ...
            diagnostic: {
                code: 'NONCE_INVALID_INPUT',
                message: 'nonce_unique_per_signer_window: state.window_seconds must be a ' +
    ...
    if (state === undefined) {
        return {
            ...
            diagnostic: {
                ...
                message: `nonce_unique_per_signer_window: nonceWindow state was not supplied via ` +
    ...
    if (seen !== undefined && !(seen instanceof Set)) {
        return {
            ...
            diagnostic: {
                code: 'NONCE_INVALID_INPUT',
                message: 'nonce_unique_per_signer_window: state.per_signer.get(...) must ' +
    ...
    if (seen && seen.has(nonce)) {
        return {
            ...
            diagnostic: {
                ...
                message: `nonce_unique_per_signer_window: nonce "${nonce}" already observed for ` +
...
//# sourceMappingURL=nonce-unique-per-signer-window.js.map