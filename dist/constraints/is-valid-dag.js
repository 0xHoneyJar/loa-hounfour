 ...
 */
export function extractPath(node, path) {
 ...
 */
export function evaluateIsValidDag(items, idField, refFields) {
        ...
        if (ops > IS_VALID_DAG_OP_CAP) {
            return {
                ...
                diagnostic: {
                    ...
                    path: '$',
                    context: { ops, phase: 'indexing', processed: i },
                },
            };
        }
        const item = items[i];
    ...
    function visit(nodeId, path) {
        ...
        if (ops > IS_VALID_DAG_OP_CAP) {
            diagnostic = {
                code: 'DAG_OP_CAP_EXCEEDED',
                path: '$',
                context: { ops, phase: 'dfs', at_node: nodeId },
            };
            exceededOpCap = true;
            return;
        }
        ...
        for (const refField of refFields) {
            ...
            if (ops > IS_VALID_DAG_OP_CAP) {
                diagnostic = {
                    code: 'DAG_OP_CAP_EXCEEDED',
                    path: '$',
                    context: { ops, phase: 'ref-resolve', at_node: nodeId },
                };
                exceededOpCap = true;
                return;
            }
...
//# sourceMappingURL=is-valid-dag.js.map