...
class Parser {
    ...
    context;
    functions;
    constructor(tokens, data, depth = 0, context) {
        this.tokens = tokens;
        this.data = data;
        this.depth = depth;
    ...
    parseFieldPath() {
        ...
        while (this.peek()?.type === 'bracket' && this.peek()?.value === '[') {
            ...
            if (indexTok?.type !== 'number')
                throw new Error('Expected numeric index in array access');
            this.expect('bracket', ']');
            ...
            if (this.peek()?.type === 'dot') {
                ...
                if (restTok?.type === 'ident') {
                    ...
                    if (element != null && typeof element === 'object') {
                        return resolve(element, restPath);
                    }
                    return undefined;
                }
            }
        ...
        while (this.peek()?.type === 'dot') {
            ...
            if (next.type === 'ident' && next.value === 'every') {
                ...
                if (paramTok.type !== 'ident')
                    throw new Error('Expected parameter name in .every()');
    ...
    findMatchingParen() {
        ...
        while (p < this.tokens.length && depth > 0) {
            if (this.tokens[p].type === 'paren' && this.tokens[p].value === '(')
                depth++;
            if (this.tokens[p].type === 'paren' && this.tokens[p].value === ')')
                depth--;
            if (depth > 0)
    ...
    parseChanged() {
        ...
        this.expect('paren', ')');
        const prev = this.data._previous;
        if (prev == null)
            return false;
     ...
     */
    parseFieldPathString() {
        const tok = this.advance();
        if (tok.type !== 'ident')
            throw new Error(`Expected identifier, got ${tok.type} "${tok.value}"`);
        let path = tok.value;
        while (this.peek()?.type === 'dot') {
            this.advance(); // consume '.'
            const next = this.peek();
    ...
    parseLinksSubsetAuthority() {
        ...
        for (let i = 1; i < links.length; i++) {
            ...
            if (!Array.isArray(parentScope) || !Array.isArray(childScope))
                return false;
            const parentSet = new Set(parentScope);
            for (const perm of childScope) {
     ...
     */
    parseDelegationBudgetConserved() {
        ...
        for (let i = 0; i < links.length - 1; i++) {
            ...
            if (parentBudget == null || childBudget == null)
                continue;
            try {
                if (BigInt(String(childBudget)) > BigInt(String(parentBudget)))
                    return false;
            }
            catch {
                return false;
    ...
    parseLinksFormChain() {
        ...
        const links = val;
        for (let i = 0; i < links.length - 1; i++) {
            if (links[i]?.delegatee !== links[i + 1]?.delegator)
                return false;
        }
        return true;
    }
    ...
    parseNoEmergentInIndividual() {
        ...
        this.expect('paren', ')');
        if (!Array.isArray(emergent))
            return true;
        if (individual == null || typeof individual !== 'object')
            return true;
        const record = individual;
        const allIndividual = new Set();
        ...
        for (const cap of emergent) {
            if (typeof cap === 'string' && allIndividual.has(cap))
                return false;
        }
        return true;
    }
    ...
    parseAllEmergentHaveEvidence() {
        ...
        if (!Array.isArray(evidence))
            return false;
        const evidencedCaps = new Set();
        for (const e of evidence) {
            if (e != null && typeof e === 'object') {
                const cap = e.capability;
                if (typeof cap === 'string')
                    evidencedCaps.add(cap);
            }
        }
        for (const cap of emergent) {
            if (typeof cap === 'string' && !evidencedCaps.has(cap))
                return false;
        }
        return true;
    }
    ...
    parseUniqueValues() {
        ...
        this.expect('paren', ')');
        if (!Array.isArray(arr) || typeof field !== 'string')
            return true;
        const seen = new Set();
    ...
    parseTreeBudgetConserved() {
        ...
        while (stack.length > 0) {
            ...
            for (const child of children) {
                childSum += BigInt(String(child?.budget_allocated_micro ?? '0'));
            }
            if (childSum > parentBudget)
                return false;
            for (const child of children) {
    ...
    parseTreeAuthorityNarrowing() {
        ...
        const stack = [{
                node: root,
                parentScope: new Set((Array.isArray(root.authority_scope) ? root.authority_scope : []).map((s) => String(s).toLowerCase())),
                depth: 0,
            }];
        let nodeCount = 0;
        while (stack.length > 0) {
            ...
            const children = Array.isArray(node.children) ? node.children : [];
            for (const child of children) {
                const childScopeArr = (Array.isArray(child?.authority_scope) ? child.authority_scope : [])
                    .map((s) => String(s).toLowerCase());
                const childScopes = new Set(childScopeArr);
    ...
    parseMonetaryPolicySolvent() {
        ...
        if (typeof ceiling !== 'string' || typeof currentSupply !== 'string')
            return false;
        try {
    ...
    parsePermissionBoundaryActive() {
        ...
        if (typeof rec.permitted_if !== 'string' || rec.permitted_if.length === 0)
            return false;
        // Must have reporting object
        if (rec.reporting == null || typeof rec.reporting !== 'object')
            return false;
        // Must have revocation object
        if (rec.revocation == null || typeof rec.revocation !== 'object')
            return false;
        return true;
    }
    // ---------------------------------------------------------------------------
    ...
    parseProposalQuorumMet() {
        ...
        if (voting == null || typeof voting !== 'object')
            return false;
        const vr = voting;
    ...
    parseSagaTimeoutValid() {
        ...
        if (typeof perStepSeconds !== 'number' || perStepSeconds <= 0)
            return false;
        // Resource limit: max 100 steps
        if (steps.length > 100)
            return false;
        for (const step of steps) {
            ...
            if (typeof s.started_at !== 'string' || typeof s.completed_at !== 'string')
                return false;
            ...
            try {
                ...
                if (durationSeconds > perStepSeconds)
                    return false;
            }
            catch {
                return false;
     ...
     * tolerance of 0.001).
     */
    parseProposalWeightsNormalized() {
    ...
    parseProposalExecutionValid() {
        ...
        if (!Array.isArray(changesApplied) || changesApplied.length === 0)
            return false;
    ...
    parseModelRoutingEligible() {
        ...
        this.expect('paren', ')');
        if (typeof qualifyingState !== 'string' || typeof currentState !== 'string')
            return false;
        if (typeof qualifyingScore !== 'number' || typeof currentScore !== 'number')
            return false;
    ...
    parseExecutionCheckpointValid() {
        ...
        if (typeof health !== 'string' || typeof decision !== 'string')
            return false;
        switch (health) {
            case 'healthy':
    ...
    parseAuditTrailChainValid() {
        ...
        if (!Array.isArray(entries) || typeof genesisHash !== 'string')
            return false;
        if (entries.length === 0)
            return true;
    ...
    parseIso8601Ms(value) {
        const s = String(value);
        if (!Parser.ISO_8601_RE.test(s))
            return NaN;
    ...
    parseTimestampBetween() {
        ...
        if (isNaN(valueMs) || isNaN(lowerMs) || isNaN(upperMs))
            return false;
    ...
    parseTimestampStaleness(op) {
        ...
        if (isNaN(timestampMs) || isNaN(referenceMs) || isNaN(maxAge) || maxAge < 0)
            return false;
 ...
 * @since v7.8.0
 */
export const RESERVED_EVALUATOR_NAMES = new Set([
    ...EVALUATOR_BUILTINS,
 ...
 */
export function evaluateConstraint(data, expression, context) {
