 ...
 */
export function computeCostMicro(pricing, usage) {
    ...
    const totalNumerator = inputRate * promptTokens +
        outputRate * completionTokens +
        thinkingRate * reasoningTokens;
    // Zero special case
    if (totalNumerator === 0n)
        return '0';
 ...
 */
export function computeCostMicroSafe(pricing, usage) {
    try {
 ...
 */
export function verifyPricingConservation(billing, usage) {
...
// Internal helpers
// ---------------------------------------------------------------------------
function validatePricingString(value, label) {
