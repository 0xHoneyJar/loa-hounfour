 ...
 */
export function extractReferences(schema, schemaId) {
    const refs = [];
    const properties = schema.properties;
    for (const [fieldName, fieldSchema] of Object.entries(properties)) {
        walkSchemaNode(fieldSchema, schemaId, fieldName, refs, 0);
    }
    return refs;
}
/**
 ...
 */
function walkSchemaNode(node, schemaId, fieldPath, refs, depth) {
        ...
        for (const ref of xRefs) {
            if (ref && typeof ref === 'object' && 'target_schema' in ref && 'target_field' in ref) {
                refs.push({
                    ...
                    relationship: String(ref.relationship ?? 'references'),
                });
            }
        }
    }
    ...
    if (node.type === 'object' && node.properties != null && typeof node.properties === 'object') {
        for (const [nestedField, nestedSchema] of Object.entries(node.properties)) {
            if (nestedSchema != null && typeof nestedSchema === 'object') {
                walkSchemaNode(nestedSchema, schemaId, `${fieldPath}.${nestedField}`, refs, depth + 1);
            }
        }
    }
    // Recurse into array items
    if (node.type === 'array' && node.items != null && typeof node.items === 'object') {
        walkSchemaNode(node.items, schemaId, `${fieldPath}[]`, refs, depth + 1);
    }
        ...
        if (Array.isArray(compositions)) {
            for (let i = 0; i < compositions.length; i++) {
                ...
                if (comp != null && typeof comp === 'object') {
                    walkSchemaNode(comp, schemaId, fieldPath, refs, depth + 1);
                }
            }
        }
    }
...
function toAdjacencyList(graph) {
    ...
    for (const node of graph) {
        if (!adj.has(node.schema_id))
            adj.set(node.schema_id, new Set());
        for (const ref of node.outgoing_references) {
            adj.get(node.schema_id).add(ref.target_schema);
            if (!adj.has(ref.target_schema))
                adj.set(ref.target_schema, new Set());
        }
    }
    return adj;
}
 ...
 */
function toReverseAdjacencyList(graph) {
    const rev = new Map();
    for (const node of graph) {
        if (!rev.has(node.schema_id))
            rev.set(node.schema_id, new Set());
        for (const ref of node.incoming_references) {
            if (!rev.has(ref.source_schema))
                rev.set(ref.source_schema, new Set());
 ...
 */
export function isReachable(graph, from, to) {
    if (from === to)
        return true;
 ...
 * @see SDD §2.4.1 — Schema Graph Reachability
 */
export function reachableFrom(graph, source) {
    const adj = toAdjacencyList(graph);
    const visited = new Set();
    const queue = [source];
    visited.add(source);
 ...
 */
export function analyzeImpact(graph, schemaId, constraintFiles) {
    const rev = toReverseAdjacencyList(graph);
    // Direct: schemas that directly reference this schema
    const directSet = rev.get(schemaId) ?? new Set();
    ...
    const visited = new Set([schemaId, ...directSet]);
    while (queue.length > 0) {
        const current = queue.shift();
