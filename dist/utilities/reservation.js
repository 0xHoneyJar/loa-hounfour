 ...
 * @see ROUNDING_BIAS — 'rights_holder' policy documentation
 */
export function computeReservedMicro(limitMicro, bps) {
 ...
 */
export function validateReservationTier(conformanceLevel, actualBps, config) {
 ...
 */
function checkAdvisoryWarning(postTransaction, reserved, thresholdPercent = ADVISORY_WARNING_THRESHOLD_PERCENT) {
    if (reserved <= 0n)
        return null;
 ...
 */
function handleFloorBreach(enforcement, postTransaction, reserved) {
...
// ---------------------------------------------------------------------------
function handleAtFloor(enforcement, available, reserved) {
 ...
 * @see ROUNDING_BIAS — 'rights_holder' policy documentation
 */
export function shouldAllowRequest(availableMicro, costMicro, reservedMicro, enforcement, config) {
