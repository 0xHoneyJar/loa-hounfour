...
const AssertionCommonFields = {
    ...
    body_hash: Type.String({
        ...
        description: 'SHA-256 hex digest (lowercase) of safeCanonicalize(assertion-body). NFC + RFC 8785 + 100KB cap per the hashing-spec freeze. The body is the assertion content the signer commits to.',
    }),
    provenance: Type.Array(ClaimGroundingSchema, {
        description: 'Per-grounding provenance records describing how the assertion content was sourced. ClaimGrounding is EXTENDED in PR-A2.3 with 1-2 substrate-agnostic discriminator members.',
    }),
...
const SignaturesArrayField = {
    signatures: Type.Array(SignatureEnvelopeSchema, {
        minItems: 1,
        description: 'At least one SignatureEnvelope. Each envelope carries the signed_payload_hash committing to body_hash + the rest of the assertion fields (consumer-side reconstruction).',
    }),
};
const CandidateAssertionVariant = Type.Object({
    ...
    description: 'candidate variant — pre-admission; no signatures[]. NOT crypto-bearing — validate() returns { valid: true } without acceptDeferred.',
});
...
const ForgottenAssertionVariant = Type.Object({
    ...
    additionalProperties: false,
    'x-crypto-bearing': true,
    description: 'forgotten variant — signatures preserved when forget_scope ∈ {pii_only, agent_full, pii_and_link_to_key}; absent only when forget_scope == crypto_full_destruction (consumer-side state). Crypto-bearing.',
});
...
export const AssertionSchema = Type.Union([
    ...
    $id: 'Assertion',
    description: 'Signed-observation-with-lifecycle envelope. 8-variant status-discriminated union. CandidateAssertion folded as status:candidate (F3 fold). Variant-aware crypto-bearing (J3): candidate is NOT crypto-bearing; the other 7 variants ARE — validate() inspects payload status to apply the safe-by-default branch. State-machine rules (A1/A2/A3) live in constraints/Assertion.constraints.json.',
});
