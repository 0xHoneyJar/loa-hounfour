...
create_manifest() {
  ...
  local upstream_version="1.7.1"
  if [[ -f ".loa-version.json" ]]; then
    upstream_version=$(jq -r '.framework_version // "1.7.1"' .loa-version.json 2>/dev/null)
  elif [[ -f ".claude/.loa-version.json" ]]; then
    upstream_version=$(jq -r '.framework_version // "1.7.1"' .claude/.loa-version.json 2>/dev/null)
  fi
...
main() {
  echo ""
  log "======================================================================="
  log "  Loa Framework Mount v1.7.1"
