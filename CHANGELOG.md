...
- **Recall Wedge composition surface** — documentation + conformance corpus describing how the existing v8.5.0 PR-A2.3 surface (`Assertion` / `Recall*` / `Forget` / `Commitment*` / `AgentEstate*`) plus the v8.6.0 PR-A3.7 `Challenge` layer compose into the Straylight Recall Wedge contract. **No new schemas, no `$id` additions, no exports changed** — the wedge consumes existing primitives.
  ...
  - **`tests/vectors/recall-wedge-vectors.test.ts`**: per-schema structural validation (with `assertStructurallyValid` for the crypto-bearing variants) plus cross-record composition-invariant assertions for every link in the round-trip flow.
