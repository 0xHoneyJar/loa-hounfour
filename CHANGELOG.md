...
- **Recall Wedge composition surface** — documentation + conformance corpus describing how the existing v8.5.0 PR-A2.3 surface (`Assertion` / `Recall*` / `Forget` / `Commitment*` / `AgentEstate*`) plus the v8.6.0 PR-A3.7 `Challenge` layer compose into the Straylight Recall Wedge contract. **No new `$id`-bearing schemas, no exports changed** — the wedge consumes existing primitives; the only TypeBox-side change is one additive `Type.Literal('recall-wedge')` member on `ConformanceCategorySchema`. Composes cleanly with the cycle-007 schemas landed in #103–#109 (`ClusterRunSeriesSchema`, `InterSeriesScopingArtifactSchema`, `SubscriptionPoolStateSchema`, `RevocationListSchema`, `MergeArtifactSchema`) — the wedge does not touch any of them. The hoisted-`SCHEMAS` single-source-of-truth introduced in #109 covers `conformance-vector.schema.json` regeneration; no `scripts/check-schemas.ts` edit is required.
  - **`docs/architecture/recall-wedge-composition.md`**: requirement-to-primitive map, round-trip flow diagram, audit/event vocabulary, canonicalization path, crypto-bearing safe-by-default reminders, and the `EstateTransition`-deferred substitute (`AgentEstate` + `AgentEstateStatus` + `CommitmentRoot{commitment_type:'transition_bundle'}`).
  - **`vectors/conformance/recall-wedge/`** (5 fixtures + README): end-to-end round-trip corpus threading deterministic cross-record references — `assertion-admitted.json` → `recall-request.json` → `recall-pack.json` → `recall-receipt.json` → `commitment-root.json`. Lazy-link string equalities (`pack.recall_request_ref === request.request_id`, `receipt.pack_hash === pack.pack_hash`, `commitment.subject_hash === receipt.receipt_hash`) match the SDD §1.5 lazy-link convention.
  - **`tests/vectors/recall-wedge-vectors.test.ts`**: per-schema structural validation (with `assertStructurallyValid` for the crypto-bearing variants) plus cross-record composition-invariant assertions for every link in the round-trip flow. Includes `recall_scope === surface_context` equality assertion between the admitted-assertion entry-point and the recall request.
  - **`AUDIT_EVENT_TYPES_KNOWN_PREFIXES`**: registers `'0xhoneyjar:straylight:'` in the soft prefix registry. The conventional event-type vocabulary for the wedge (`assertion.{admitted,challenged,revoked,forgotten}`, `estate.transition.applied`, `recall.{request.received,pack.assembled,receipt.signed}`, `commitment.anchored`) is documented as a comment block above the registry. The runtime registry is keyed by prefix, not by event type — the listed types remain consumer-mintable. `tests/vocabulary/audit-event-types.test.ts` extended with structural-validation assertions for each conventional type.
  - **`ConformanceCategorySchema`**: appends `'recall-wedge'` as the 47th category literal (was 46) so vectors under `vectors/conformance/recall-wedge/` satisfy the Category-Directory Consistency check in `tests/vectors/conformance-validation.test.ts`.
